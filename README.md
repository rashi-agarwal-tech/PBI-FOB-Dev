# Introduction 
All of the DMA Power BI dataset reside in this repository. 
...
- [Deployment:](#deployment)
  - [Deployment Notes](#deployment-notes)
    ...
    - [Current defeciencies:](#current-defeciencies)
...
Once changes have been merged to the `main` branch of the repository, [windermere_powerbi](#https://projectreboot.visualstudio.com/Windermere%20Discovery/_git/windermere_powerbi/pullrequests?_a=mine), you should be able to see the changeson the `Source control` button in the UI as you can see below. Clicking on the button will opend up a new panel on the right hand side with all changes that you can see in the in the second image below. The `Source control` panel should show you any changes that require to sync in both directions but I would advise not making any changes outside of the repository directly, it should be a oneway process, from repository to power bi.
...
![pipline](docs/images/pipeline.PNG)
...
![pipline](docs/images/pipeline_compare_btn.PNG)

Clicking on `Compare` allows you to see the differences between the two enviornments at a glance and also allows you to be selective about which reports you want to deploy to the next stage, as you can see in the below image I have only selected `DMA Operations` for deployment. Note that you cannot see these selection boxes until you hover over the artifact. Infact there are also chance to review the changes in a difference comparison window that will also appear so that you can check the changes to be deployed. This is also possible to access directly from the main dropdown which can be seen in the above image.
...
If you do try to deploy at these times the deployment will most likely fail gracfully but you will just get the loading spinner graphic until the deployment times out which can be quite some time.

### Report Unique Identifier
Each model, report, object is given a unquie id, it is important to maintain these IDs as some user will directly refrer to the report using its' unique ID. An objects ID will be maintained if the object is being updated by the standard process but if a report or model is deleted this ID will be lost potentially breaking end user reports in Excel.

### Current defeciencies:
It is not currently possible to deploy singlar changes to a report where multple changes exist in the development workspace as the pipeline can only deploy at the report level. 
...
![pipline](docs/images/pbi_report_format.PNG)

### Time Intelligence
Time Intelligence is a feature that will add a date time dimension table automatically for every date or date time column that is present in any query. This is generally not require and is adviced to be switched off in the setting, `File -> Options and settings -> Options` and ensure that the below highlighted check box is **not** selected, `Auto date/time for new files`.

![pipline](docs/images/pbi_time_intelligence.PNG)
...
Any reports that have custom partitions will have to be swapped out to enable you to open the report. If this is not performen the report will fail to open in Power BI Desktop with the below error, `Sequence conatians more than one element`
...
It is recommended to install a python virtual environment, this can be accomplish in many ways with the package managers mentioned above. Currently `poetry` provides a simple way to manage dependancies and install the current project for you. `pipx` is usefull for globally installing packages into there own discreate environments.
...
```
 ...
 You will need to activate the environment which is dependant upone your operating system and then install from the pyproject.toml file as follows.
 ```
 ...
 Once you have installed the depenancies for the project and installed the current package you should be able to use the command line utility `pbp`. You can check available commands as follows:
